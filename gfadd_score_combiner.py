...
def form_combined_scores(gfadd_scores_filename, combined_scores_filename):
    start_time = time.time()
    print "Reading scores data from " + gfadd_scores_filename
    # Weights for each plot from 1-8 respectively
    weights = [1, 1, 1, 1, 1/3 ,1/3 ,1/3, 1/3, 1/3]

    gfadd_scores_data = pd.read_csv(gfadd_scores_filename, skipinitialspace=True, escapechar="\\", header=None)

    scores = [gfadd_scores_data, ]
...
def main():
    ...
    form_combined_scores(gfadd_scores_filename, combined_scores_filename)
