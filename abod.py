...
def angleBAC(A, B, C, AB, AC):				# AB AC mold
	...
	if math.fabs(cos_AB_AC_) > 1:
		...
		# print '|AB| * |AC| = ', AB * AC
		# print 'cos_AB_AC_ = ', math.acos(cos_AB_AC_)
...
#			print '100 points, START %s\tEND %s' % (startTime, getCurrTime()) 
	...
	# MODIFIED FROM ORIGINAL
	ABOD_scores = []
	for key, value in DictABOF.iteritems():
		ABOD_scores.append(value)
...
def main():
	...
	fSrc = "combined_data_short.csv"

	fSampledFile = 'StatusData%d.csv' % N
	...
	fABOD = 'ABOD_%d__%d_%s.csv' % (N, topK, currTime)
